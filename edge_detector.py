...
class EdgeDetector(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        out = F.conv2d(x.unsqueeze(1), self.kernel.unsqueeze(0).unsqueeze(0), padding=self.padding)
        return out
...
def lap_of_gaus(x, y, sigma=1.0):
    ...
    """
    return -1.0 / (math.pi * sigma ** 2) * (1 - (x**2 + y**2) / (2 * sigma **2 )) * math.exp(-1.0 * (x**2 + y**2) / (2.0 * sigma**2))
